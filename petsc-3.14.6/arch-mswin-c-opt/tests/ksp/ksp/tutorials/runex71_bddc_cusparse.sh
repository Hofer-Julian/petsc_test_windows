...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex71_bddc_cusparse.out ex71_bddc_cusparse.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
