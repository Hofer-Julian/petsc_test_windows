...
for insize in ${nsize_in}; do
   for idirichlet in ${dirichlet_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex71_aij_dmda_preall.out ex71_aijcusparse_dmda_preall.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+dirichlet-${idirichlet} ""
      else
