...
for insize in ${nsize_in}; do
   for itest_custom_layout in ${test_custom_layout_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex27_6_hdf5_dense.out ex27_6_hdf5_dense.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}test_custom_layout-${itest_custom_layout} ""
      else
