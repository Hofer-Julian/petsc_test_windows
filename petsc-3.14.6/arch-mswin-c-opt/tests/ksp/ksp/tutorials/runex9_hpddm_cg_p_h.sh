...
for insize in ${nsize_in}; do
   for is2_ksp_hpddm_type in ${s2_ksp_hpddm_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex9_hpddm_cg.out ex9_hpddm_cg_p_h.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+s2_ksp_hpddm_type-${is2_ksp_hpddm_type} ""
      else
