...
for insize in ${nsize_in}; do
   for imat_type in ${mat_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex77_4.out ex77_4.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_type-${imat_type} ""
      else
