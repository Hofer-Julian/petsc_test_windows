...
for insize in ${nsize_in}; do
   for imatis_localmat_type in ${matis_localmat_type_in}; do
      for ipc_hpddm_coarse_mat_type in ${pc_hpddm_coarse_mat_type_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex71_hpddm.out ex71_hpddm.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+matis_localmat_type-${imatis_localmat_type}_pc_hpddm_coarse_mat_type-${ipc_hpddm_coarse_mat_type} ""
         else
