...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex11_deflation.out ex11_deflation.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+a ""
   else
...
for insize in ${nsize_in}; do
   for ipc_deflation_compute_space in ${pc_deflation_compute_space_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tutorials/output/ex11_deflation.out ex11_deflation.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+b+pc_deflation_compute_space-${ipc_deflation_compute_space} ""
      else
         petsc_report_tapoutput "" ${label} "SKIP Command failed so no diff"
      fi
