...
for insize in ${nsize_in}; do
   for iksp_type in ${ksp_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ksp/ksp/tests/output/ex6_4_ksp_norm_type-natural_pc_type-bjacobi.out ex6_4_ksp_norm_type-natural_pc_type-bjacobi.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ksp_type-${iksp_type} ""
      else
