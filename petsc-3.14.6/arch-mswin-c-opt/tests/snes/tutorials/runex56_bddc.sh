...
for insize in ${nsize_in}; do
   for imatis_localmat_type in ${matis_localmat_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/snes/tutorials/output/ex56_bddc.out ex56_bddc.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+matis_localmat_type-${imatis_localmat_type} ""
      else
