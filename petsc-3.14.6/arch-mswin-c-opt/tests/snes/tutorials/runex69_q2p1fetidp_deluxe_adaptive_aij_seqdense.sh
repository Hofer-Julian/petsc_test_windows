...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/snes/tutorials/output/ex69_q2p1fetidp_deluxe_adaptive.out ex69_q2p1fetidp_deluxe_adaptive_aij_seqdense.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
