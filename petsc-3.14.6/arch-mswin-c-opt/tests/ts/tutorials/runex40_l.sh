...
for insize in ${nsize_in}; do
   for its_adapt_always_accept in ${ts_adapt_always_accept_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ts/tutorials/output/ex40.out ex40_l.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ts_adapt_always_accept-${its_adapt_always_accept} ""
      else
