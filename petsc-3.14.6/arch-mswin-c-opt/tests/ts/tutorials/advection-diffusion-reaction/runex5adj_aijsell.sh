...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/ts/tutorials/advection-diffusion-reaction/output/ex5adj_sell_1.out ex5adj_aijsell.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
