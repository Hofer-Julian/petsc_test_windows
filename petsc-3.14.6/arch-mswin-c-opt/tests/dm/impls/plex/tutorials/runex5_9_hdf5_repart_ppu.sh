...
exec='../ex5'
testname='runex5_9_hdf5_repart_ppu'
label='dm_impls_plex_tutorials-ex5_9_hdf5_repart_ppu'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex5_9_hdf5_repart_ppu.tmp ${testname}.err "${label}+nsize-${insize}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tutorials/output/ex5_9_hdf5_repart_ppu.out ex5_9_hdf5_repart_ppu.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize} ""
   else
