...
exec='../ex5'
testname='runex5_9_hdf5_seqload'
label='dm_impls_plex_tutorials-ex5_9_hdf5_seqload'
runfiles=''
...
for insize in ${nsize_in}; do
   for iinterpolate in ${interpolate_in}; do
      ...
      petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args}  -interpolate ${iinterpolate}" ex5_9_hdf5_seqload.tmp ${testname}.err "${label}+nsize-${insize}interpolate-${iinterpolate}" 
      res=$?

      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tutorials/output/ex5_9_hdf5_seqload.out ex5_9_hdf5_seqload.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}interpolate-${iinterpolate} ""
      else
