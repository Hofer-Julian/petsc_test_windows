...
for insize in ${nsize_in}; do
   for imsh in ${msh_in}; do
      for iorder in ${order_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex99_B2_bnd.out ex99_B2_bnd.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+msh-${imsh}_order-${iorder} ""
         else
