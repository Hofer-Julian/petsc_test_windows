...
for insize in ${nsize_in}; do
   for ifmt in ${fmt_in}; do
      for ibin in ${bin_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex99_msh-vtx.out ex99_msh-vtx.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+fmt-${ifmt}_bin-${ibin} ""
         else
