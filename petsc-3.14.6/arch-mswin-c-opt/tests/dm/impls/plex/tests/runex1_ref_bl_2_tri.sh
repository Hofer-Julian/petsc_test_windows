...
for insize in ${nsize_in}; do
   for iext_hfirst in ${ext_hfirst_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex1_ref_bl_2_tri.out ex1_ref_bl_2_tri.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ext_hfirst-${iext_hfirst} ""
      else
