      ...
      for ipartitioning in ${partitioning_in}; do
         for irepartitioning in ${repartitioning_in}; do
            for itpweight in ${tpweight_in}; do
               ...
               if test $res = 0; then
                  petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex24_4.out ex24_4.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}faces-${ifaces}_partitioning-${ipartitioning}_repartitioning-${irepartitioning}_tpweight-${itpweight} ""
               else
