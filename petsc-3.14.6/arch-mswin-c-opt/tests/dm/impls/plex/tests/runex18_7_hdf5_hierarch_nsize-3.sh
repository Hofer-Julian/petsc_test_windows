...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex18_7_hdf5_hierarch_nsize-3.out ex18_7_hdf5_hierarch_nsize-3.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
