   ...
   for iorder in ${order_in}; do
      for ifmt in ${fmt_in}; do
         for ibin in ${bin_in}; do
            ...
            if test $res = 0; then
               petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/impls/plex/tests/output/ex99_msh-tet.out ex99_msh-tet.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+order-${iorder}_fmt-${ifmt}_bin-${ibin} ""
            else
