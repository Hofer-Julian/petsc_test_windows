...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/partitioner/tests/output/ex33_ptscotch_strategy_petscpartitioner_ptscotch_strategy-QUALITY.out ex33_ptscotch_strategy_petscpartitioner_ptscotch_strategy-QUALITY.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize} ""
   else
