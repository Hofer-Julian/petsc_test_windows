...
for insize in ${nsize_in}; do
   for ipwgts in ${pwgts_in}; do
      for ivwgts in ${vwgts_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/partitioner/tests/output/ex33_parmetis_nsize-1_nparts-2.out ex33_parmetis_nsize-1_nparts-2.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+pwgts-${ipwgts}_vwgts-${ivwgts} ""
         else
