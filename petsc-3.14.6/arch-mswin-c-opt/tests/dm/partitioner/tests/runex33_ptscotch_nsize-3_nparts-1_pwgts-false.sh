...
for insize in ${nsize_in}; do
   for ivwgts in ${vwgts_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/dm/partitioner/tests/output/ex33_ptscotch_nsize-3_nparts-1_pwgts-false.out ex33_ptscotch_nsize-3_nparts-1_pwgts-false.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+vwgts-${ivwgts} ""
      else
