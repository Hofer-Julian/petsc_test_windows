...
for insize in ${nsize_in}; do
   for imat_composite_merge in ${mat_composite_merge_in}; do
      for imat_composite_merge_mvctx in ${mat_composite_merge_mvctx_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tutorials/output/ex9_1.out ex9_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_composite_merge-${imat_composite_merge}_mat_composite_merge_mvctx-${imat_composite_merge_mvctx} ""
         else
