...
for insize in ${nsize_in}; do
   for iov in ${ov_in}; do
      for imat_block_size in ${mat_block_size_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex92_1.out ex92_3.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}ov-${iov}_mat_block_size-${imat_block_size} ""
         else
