...
for insize in ${nsize_in}; do
   for ibs in ${bs_in}; do
      for iexpl_type in ${expl_type_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex222_null.out ex222_matexpl_square.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}bs-${ibs}_expl_type-${iexpl_type} ""
         else
