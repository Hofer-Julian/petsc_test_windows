   ...
   for in in ${n_in}; do
      for idim in ${dim_in}; do
         for isymm in ${symm_in}; do
            ...
            if test $res = 0; then
               petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex66_1.out ex66_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+n-${in}_dim-${idim}_symm-${isymm} ""
            else
