   ...
   for in in ${n_in}; do
      for ibgpu in ${bgpu_in}; do
         for iagpu in ${agpu_in}; do
            ...
            if test $res = 0; then
               petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex66_2.out ex66_2_cuda.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+n-${in}_bgpu-${ibgpu}_agpu-${iagpu} ""
            else
