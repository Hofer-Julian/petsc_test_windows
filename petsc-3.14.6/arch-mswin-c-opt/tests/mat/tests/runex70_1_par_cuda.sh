      ...
      for ixgpu in ${xgpu_in}; do
         for ibgpu in ${bgpu_in}; do
            for iA_mat_type in ${A_mat_type_in}; do
               ...
               if test $res = 0; then
                  petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex70_1.out ex70_1_par_cuda.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+local-${ilocal}_xgpu-${ixgpu}_bgpu-${ibgpu}_A_mat_type-${iA_mat_type} ""
               else
