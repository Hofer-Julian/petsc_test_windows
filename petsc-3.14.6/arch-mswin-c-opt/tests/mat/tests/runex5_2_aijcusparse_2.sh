...
for insize in ${nsize_in}; do
   for isf_type in ${sf_type_in}; do
      for ivecscatter_packongpu in ${vecscatter_packongpu_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex5_23.out ex5_2_aijcusparse_2.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+sf_type-${isf_type}_vecscatter_packongpu-${ivecscatter_packongpu} ""
         else
