   ...
   for itestlayout in ${testlayout_in}; do
      for ibgpu in ${bgpu_in}; do
         for icgpu in ${cgpu_in}; do
            ...
            if test $res = 0; then
               petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/mat/tests/output/ex66_1_par.out ex66_1_par_cuda.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+testlayout-${itestlayout}_bgpu-${ibgpu}_cgpu-${icgpu} ""
            else
