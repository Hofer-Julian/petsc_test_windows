...
for insize in ${nsize_in}; do
   for iexplicit_inverse in ${explicit_inverse_in}; do
      for isf_window_sync in ${sf_window_sync_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/vec/is/sf/tests/output/ex5_2.out ex5_2_window_shared.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+explicit_inverse-${iexplicit_inverse}_sf_window_sync-${isf_window_sync} ""
         else
