...
for insize in ${nsize_in}; do
   for isf_window_sync in ${sf_window_sync_in}; do
      for isf_window_flavor in ${sf_window_flavor_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /cygdrive/c/checkouts/modflow/petsc-3.14.6/src/vec/is/sf/tutorials/output/ex1_4.out ex1_4.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+sf_window_sync-${isf_window_sync}_sf_window_flavor-${isf_window_flavor} ""
         else
